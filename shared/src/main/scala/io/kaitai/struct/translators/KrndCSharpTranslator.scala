...
import io.kaitai.struct.languages.KrndCSharpCompiler

class KrndCSharpTranslator(provider: TypeProvider, importList: ImportList) extends BaseTranslator(provider) {
  override def doArrayLiteral(t: DataType, value: Seq[expr]): String = {
    val nativeType = KrndCSharpCompiler.kaitaiType2NativeType(t)
  ...
  override def numericBinOp(left: Ast.expr, op: Ast.operator, right: Ast.expr) = {
    (detectType(left), detectType(right), op) match {
      case (_: IntType, _: IntType, Ast.operator.Mod) =>
        s"${KrndCSharpCompiler.kstreamName}.Mod(${translate(left)}, ${translate(right)})"
      case _ =>
  ...
  def enumClass(enumTypeAbs: List[String]): String = {
    ...
    KrndCSharpCompiler.types2class(enumTypeRel)
  }
  ...
  override def doBytesCompareOp(left: Ast.expr, op: Ast.cmpop, right: Ast.expr): String =
    s"(${KrndCSharpCompiler.kstreamName}.ByteArrayCompare(${translate(left)}, ${translate(right)}) ${cmpOp(op)} 0)"
  ...
  override def doCast(value: Ast.expr, typeName: DataType): String =
    s"((${KrndCSharpCompiler.kaitaiType2NativeType(typeName)}) (${translate(value)}))"
  ...
  override def strReverse(s: expr): String =
    s"${KrndCSharpCompiler.kstreamName}.StringReverse(${translate(s)})"
